...
export default function Sync() {
    ...
    const [consulting, setConsulting] = useState(false); // Simula estado de consulta ao servidor
    ...
    async function getAllSyncData() {
        ...
        if (responseUsers.length > 0 || responseCategory.length > 0 || responseProduct.length > 0 || responsItem.length > 0) {
            ...
            setPendingCount(quantityPending)
        }
    }
    ...
    const handleConnectToggle = () => {
        ...
        SyncService.setUserConnection(!isConnected, String(userId)).then((success) => {
            ...
            if (success) {
                ...
                Alert.alert('Falha de conexão', 'A conexão foi fechada pelos administradores. Procure um administrado e tente novamente.');
            }
    ...
    // Função para enviar usuários ao servidor
    async function postUsers() {
        ...
        for (const user of users) {
            try {
                ...
                const response = await SyncService.sendUsers(user, String(userId));
            ...
            <View style={styles.content}>
                ...
                <View style={styles.actionNuvemContent}>
                    <Button 
                        ...
                        name={loading
                                ? (
                                    consulting ? ("Verificando conexão...") : (isConnected ? "Desconectando..." : "Conectando...")
                                )
